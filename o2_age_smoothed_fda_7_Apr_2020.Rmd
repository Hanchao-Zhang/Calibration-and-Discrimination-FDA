---
title: "Smoothed data Functional data analysis for o2"
output:
  html_document:
    toc: true
    toc_float: true
...
dat_demographic_age <- dat_demographic[,c(1,3)]
names(dat_demographic_age) <- c('StudyID', 'age')
...
cv_prediction_err <- function(data = dat_o2_grouped_demographic, var_int = 'mean_o2', var_adj = c('age'), y = 'survival', cv.seed = 456){
...
auc_estimation <- function(data = dat_o2_grouped_demographic, var_int = 'mean_o2', var_adj = c('age', 'CPR_time'), y = 'survival', k = 5){
  ind <- sample(nrow(data), nrow(data), replace = F)
  data <- data[ind,]
